...
def radius(points: Iterable[Tuple[float, float, float]], dt: float = 1.0) -> List[float]:
    """
...
def fidelity(points_r: Iterable[Tuple[float,float,float]],
    ...
    plt.ylabel("Fidelity")
    plt.title("Evolution of the fidelity between two trajectories")
    plt.ylim(0, 1.05)
    plt.grid(True, alpha=0.25)
    plt.show()
...
kraus_channels = [kraus_dep_discret, kraus_T1_discret, kraus_T2_discret]


pts0 = continuous_path(rho0, seq, steps_per_gate = steps_per_gate)
pts = continuous_path(rho0, seq, steps_per_gate = steps_per_gate, kraus_channels = kraus_channels, noise_start = 0, noise_end = n_seq)
